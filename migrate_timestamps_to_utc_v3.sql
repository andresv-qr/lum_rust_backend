-- =====================================================================
-- SCRIPT DE MIGRACIÓN UTC COMPLETO V3
-- Convierte todos los campos timestamp a timestamptz (UTC)
-- Basado en análisis real de la estructura de la base de datos
-- =====================================================================

BEGIN;

-- =====================================================================
-- PASO 1: GUARDAR DEFINICIONES DE VISTAS (para recrear después)
-- =====================================================================

-- Primero eliminamos todas las vistas que dependen de campos timestamp
...
-- =====================================================================
-- PASO 2: MIGRAR CAMPOS DE LOGS SCHEMA
-- =====================================================================
...
-- invoice_header_tempsinregistro
...
-- =====================================================================
-- PASO 4: MIGRAR CAMPOS DE REWARDS SCHEMA
-- =====================================================================
...
SELECT 
    surveys.survey_id,
    username,
    CASE WHEN sa.answers IS NULL THEN 0 ELSE 1 END AS completed
FROM surveys
CROSS JOIN users
...
SELECT 
    u.id, u.email, u.name, u.auth_providers, u.google_id,
...
SELECT 
    a.cufe, a.date, a.user_email, a.user_id, a.issuer_name,
    a.user_phone_number, a.user_telegram_id, a.process_date,
    a.reception_date, a.type,
    b.partkey, b.code, b.description, b.quantity,
    b.unit_price, b.unit_discount, b.amount, b.total, b.itbms
FROM (
    SELECT date, user_email, cufe, issuer_name, user_phone_number,
           user_id, user_telegram_id, process_date, reception_date, type
    FROM invoice_header
) a
JOIN (
    SELECT cufe, partkey, ltrim(code, '0') AS code, description,
           quantity::double precision AS quantity,
           unit_price::double precision AS unit_price,
           unit_discount,
           amount::double precision AS amount,
           total::double precision AS total,
           itbms::double precision AS itbms
    FROM invoice_detail
) b USING (cufe);
...
SELECT 
    ih.cufe, ih.issuer_name, ih.date, ih.process_date,
    CASE
        WHEN ih.user_telegram_id IS NULL AND vtu.telegram_id IS NULL THEN '-1'
        WHEN ih.user_telegram_id IS NULL THEN vtu.telegram_id::text
        ELSE ih.user_telegram_id
    END AS user_telegram_id,
    CASE
        WHEN ih.user_email IS NULL AND vtu.email IS NULL THEN '-1'
        WHEN ih.user_email IS NULL THEN vtu.email::text
        ELSE ih.user_email
    END AS user_email,
    ih.tot_itbms, ih.tot_amount
FROM invoice_header ih
LEFT JOIN vw_telegram_users_full vtu ON (
    ih.user_telegram_id = vtu.telegram_id::text 
    OR ih.user_email = vtu.email::text
);
...
WITH ranked_data AS (
    SELECT user_email,
        (to_char(date, 'YYYYMM01'))::date AS mes,
        sum(tot_amount) AS monto,
        count(DISTINCT cufe) AS num_facturas,
        count(DISTINCT issuer_name) AS comercios
    FROM invoice_header
    WHERE date > (CURRENT_DATE - '6 mons'::interval)
    GROUP BY user_email, (to_char(date, 'YYYYMM01'))::date
)
SELECT user_email,
...
WITH ranked_data AS (
    SELECT a.user_email,
        ...
        sum(CASE WHEN d.unit_discount = '' THEN NULL ELSE d.unit_discount::double precision END * d.quantity::double precision) AS descuentos,
...
WITH t AS (
    SELECT a.cufe, a.issuer_name, a.date, a.process_date,
           a.user_telegram_id, a.user_email, a.tot_itbms, a.tot_amount
    FROM invoice_header a
    JOIN (
        SELECT user_email, max(process_date) AS process_date
        FROM invoice_header
        GROUP BY user_email
...
WITH ranked_data AS (
    SELECT user_email, issuer_name,
        count(*) AS visitas,
        sum(tot_amount) AS monto,
        row_number() OVER (PARTITION BY user_email ORDER BY count(*) DESC) AS rank
    FROM invoice_header
    WHERE date > (CURRENT_DATE - '6 mons'::interval)
    GROUP BY user_email, issuer_name
),
rd2 AS (
    SELECT user_email, issuer_name, visitas, monto
    FROM ranked_data WHERE rank <= 5
)
SELECT user_email,
...
WITH ranked_data AS (
    SELECT a.user_email, d.description,
        sum(d.quantity::double precision) AS qty,
...
rd2 AS (
    SELECT user_email, description, qty, amt
    FROM ranked_data WHERE rank <= 10
)
SELECT user_email,
...
FROM (
    SELECT user_email,
        count(DISTINCT cufe) AS num_facturas,
        count(DISTINCT issuer_name) AS num_comercios,
        sum(tot_itbms) AS tot_itbms
    FROM invoice_header
    GROUP BY user_email
) sub
GROUP BY user_email;

-- vw_usr_general_metrics (la que se eliminó en cascada)
...
SELECT 
    ...
    l.level_id AS current_level, l.level_name, l.level_color,
    l.benefits_json AS level_benefits,
    l.min_xp AS level_min_points, l.max_xp AS level_max_points,
    COALESCE(nl.min_xp - us.total_xp, 0) AS invoices_to_next_level,
    nl.level_name AS next_level_name,
    COALESCE(
        (SELECT jsonb_object_agg(streak_type, jsonb_build_object('current', current_count, 'max', max_count))
         FROM gamification.user_streaks
         WHERE user_id = us.user_id AND is_active = true),
        '{}'::jsonb
    ) AS active_streaks,
    (SELECT count(*) FROM gamification.user_mechanics
     WHERE user_id = us.user_id AND status::text = 'active') AS active_mechanics_count
...
-- =====================================================================
-- PASO 6: ACTUALIZAR TRIGGER PARA UTC
-- =====================================================================
