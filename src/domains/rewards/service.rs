...
pub async fn send_user_metrics_dashboard(app_state: &Arc<AppState>, ws_id: &str) -> Result<()> {
    if let Some(user) = user_service::get_user(app_state, ws_id).await? {
        if let Some(email) = &user.email {
            if !email.is_empty() {
                match user_service::get_user_summary(app_state, email).await {
                Ok(Some(metrics)) => {
...
pub async fn deduct_lumis_for_ocr(pool: &PgPool, user_id: i64, cost: i32) -> Result<()> {
    ...
    // 1. Get current balance with FOR UPDATE lock
    let balance_record = sqlx::query!(
        "SELECT balance::integer FROM rewards.fact_balance_points WHERE user_id = $1 FOR UPDATE",
        user_id as i32
    )
...
impl UserSummaryService {
    ...
    pub async fn get_user_summary(
        ...
        let summary = match self.get_base_summary(user_id).await {
            Ok(s) => s,
            Err(e) => return Err(ApiError::database_error(&e.to_string()))
        };
        ...
        let performance_metrics = if query.include_trends.unwrap_or(true) {
            match self.calculate_performance_metrics(&summary).await {
                Ok(m) => m,
                Err(e) => return Err(ApiError::database_error(&e.to_string()))
            }
        } else {
        ...
        let trends = if query.include_projections.unwrap_or(true) {
            match self.calculate_trend_analysis(&summary).await {
                Ok(t) => t,
                Err(e) => return Err(ApiError::database_error(&e.to_string()))
            }
        } else {
    ...
    async fn get_base_summary(&self, user_id: i32) -> anyhow::Result<UserInvoiceSummary> {
        let query = r#"
            SELECT 
    ...
    /// Calcular métricas de rendimiento
    async fn calculate_performance_metrics(&self, summary: &UserInvoiceSummary) -> anyhow::Result<PerformanceMetrics> {
    ...
    /// Análisis de tendencias
    async fn calculate_trend_analysis(&self, summary: &UserInvoiceSummary) -> anyhow::Result<TrendAnalysis> {
    ...
    pub async fn user_has_summary(&self, user_id: i32) -> Result<bool, ApiError> {
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                Err(ApiError::database_error(&e.to_string()))
            }
        }
    }
}
