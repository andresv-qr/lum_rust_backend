...
pub async fn process_invoice_handler(
    ...
    if let Err(validation_errors) = validate_process_request(&request) {
        ...
        return Err(InvoiceProcessingError::ValidationError { message: error_message });
    }
    ...
    let log_id = logging_service.start_processing(
        ...
        InvoiceProcessingError::ValidationError { message: format!("Logging initialization failed: {}", e) }
    })?;
    ...
    if let Err(e) = save_invoice_data(&pool, &full_invoice_data).await {
    ...
    let response = create_success_response(
        full_invoice_data.header.cufe.clone(),
        full_invoice_data.header.no.clone(),
        full_invoice_data.header.issuer_name.clone(),
