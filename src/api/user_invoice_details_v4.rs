...
pub fn create_user_invoice_details_v4_router() -> Router<Arc<AppState>> {
    Router::new().route("/details", get(get_user_invoice_details))
}
...
pub async fn get_user_invoice_details(
    ...
    // Execute main query (join with invoice_header to filter by user_id)
    let details = if let Some(date_filter) = &update_date_filter {
    ...
    let deleted_items = if let Some(since) = &update_date_filter {
        get_deleted_items_since(&state.db_pool, "invoice_detail", "cufe", since).await
    } else {
        vec![]
    };
